...
| Business-logic edge cases | 🟡 ~60% | Reports have **no div-by-zero** (funnel `stageConversion` returns null on from=0; time-to-hire/queries guard length=0). Plan limits enforced on all internal creates; **public-apply → BL-203 FIXED** (soft cap: accept + notify owners at the crossing). `noUncheckedIndexedAccess`+`exactOptionalPropertyTypes` cover most null/boundary cases at type level. Remaining: offer/interview state-machine edge cases. |
