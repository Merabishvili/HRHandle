...
| Side rail for terminal stages | Mentioned but not pixel-clear | Sketch: vertical strip right of the last visible column; collapsed cards show only count + initials; click-to-expand. **Implemented** in [`components/pipeline/terminal-rail.tsx`](../../../components/pipeline/terminal-rail.tsx) — 44px collapsed rail ("Rejected N · Withdrawn N", vertical text) rendered as the **last child of the single `overflow-x-auto` board row**, so the active stages and the terminal group share one horizontal scroll (the main content column is `min-w-0` so the row scrolls within the page instead of clipping). Click-to-expand opens it INLINE as a bordered "CLOSED" group (Pipeline Closed Expanded.dc.html) — a header over Rejected & Withdrawn rendered as normal-width drop-target columns; rejected cards show a red spine + the rejection reason, closed cards are dimmed but still selectable (shared selection → the bulk bar works on them too). Auto-expands during a drag. Replaced the earlier below-board footer strip. |
| Bulk reject reason picker | Was a bare `confirm()` | Bulk **Reject** in the action bar opens [`BatchRejectionDialog`](../../../components/vacancies/batch-rejection-dialog.tsx) (reason required when reasons exist + optional templated email), delegating to `rejectApplicationsBatch`; the board mirrors the move into local state on success. |
