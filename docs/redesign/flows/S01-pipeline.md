...
| Side rail for terminal stages | Mentioned but not pixel-clear | Sketch: vertical strip right of the last visible column; collapsed cards show only count + initials; click-to-expand. **Implemented** in [`components/pipeline/terminal-rail.tsx`](../../../components/pipeline/terminal-rail.tsx) — 44px collapsed rail ("Rejected N · Withdrawn N", vertical text) **pinned outside the columns' horizontal scroll container** (a `shrink-0` sibling of the `flex-1 min-w-0 overflow-x-auto` column area) so it is always fully visible and never clips. Click-to-expand opens it INLINE as a bordered "CLOSED" group (Pipeline Closed Expanded.dc.html) — a header over Rejected & Withdrawn rendered as normal-width drop-target columns; rejected cards show a red spine + the rejection reason, closed cards are dimmed but still selectable (shared selection → the bulk bar works on them too). Auto-expands during a drag. Replaced the earlier below-board footer strip. |
