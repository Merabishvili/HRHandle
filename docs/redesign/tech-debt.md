...
### 🟡 Wave 2.6 Slice 2b+ — per-vacancy reads + Stage Manager UI

Slices 1 and 2a shipped (see [Changelog](#changelog--paid-down)). What remains across the rest of the cutover:

**Slice 2b — Per-vacancy + tail reads** (small/medium):
- Vacancy-detail pipeline view ([`app/(dashboard)/vacancies/[id]/pipeline/page.tsx`](<../../app/(dashboard)/vacancies/[id]/pipeline/page.tsx>)) — should display the vacancy's own `pipeline_stages` rows as columns, not the canonical 7. This is what makes custom stages "real" to the recruiter.
- Cascading callers: [`components/pipeline/kanban-board.tsx`](../../components/pipeline/kanban-board.tsx) accepts statuses as input — extending the prop to take stage rows (or normalising to a shared shape) is the right move alongside the vacancy-pipeline cutover.
- Tail reads: [`components/vacancies/vacancy-applications-list.tsx`](../../components/vacancies/vacancy-applications-list.tsx), [`app/(dashboard)/candidates/[id]/page.tsx`](<../../app/(dashboard)/candidates/[id]/page.tsx>) (current-stage display), [`lib/reports/queries.ts`](../../lib/reports/queries.ts), [`app/api/export/applications/route.ts`](../../app/api/export/applications/route.ts), [`lib/cache/lookups.ts`](../../lib/cache/lookups.ts).
- Bucket-mapper is already shipped ([`lib/pipeline-stages/bucket.ts`](../../lib/pipeline-stages/bucket.ts)) — reusable for any cross-vacancy reader (reports, export, dashboard tiles).
...
**Estimated effort:** ~S–M per remaining slice. Slice 1's foundation + Slice 2a's bucket-mapper drop the per-slice risk.
