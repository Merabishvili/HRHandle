...
### 🟡 Wave 2.6 Slice 2c + Slice 4 — tail reads + Migration 051

Slices 1, 2a, 2b and 3 shipped (see [Changelog](#changelog--paid-down)). What remains:
...
**Slice 4 — Migration 051** (small): drop `applications.status_id` once reads + writes have stabilised on `pipeline_stage_id` for one release. (Migration 050 went to the scorecard 1→5 fidelity fix; the column-drop migration takes the next slot.)
