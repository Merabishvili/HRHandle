...
| Wave | What | Why |
|---|---|---|
| 3.1 | AI Fit Analysis | **Shipped 2026-07-22 — opt-in, default OFF.** Full scaffolding built to the six guardrails: sanitize-by-construction (`lib/ai/cv-sanitizer.ts`), criteria-locked engine + versioned prompt (`lib/ai/fit-analysis.ts`), geofence (`lib/ai/fit-geofence.ts`), server actions (`lib/actions/ai-fit.ts`), the collapsed advisory profile card with mandatory Agree/Override (`ai-fit-card.tsx`), owner opt-in with EU acknowledgement (Settings → Organization), applicant disclosure on the apply form, and an admin bias-oversight surface (Settings → Data → AI oversight). No overall score; no AI output in reports/lists/bulk/CSV. Flow: [`flows/S11-ai-fit-analysis.md`](flows/S11-ai-fit-analysis.md). **Each org still completes its own Phase 0.8 legal review before flipping it on — the acknowledgement gate records that.** See [`ai-fit-analysis.md`](ai-fit-analysis.md). |
...
| A-2 (settings audit) | Security sessions / Team management / Profile / Notifications fixes | **Shipped 2026-07-03** — Per `FIX-PROMPT-Settings-DeepAudit`. (1) **Security → Active sessions** no longer shows "No active sessions" while signed in: the security page synthesises the current session from the request user-agent when the `list_my_sessions` RPC doesn't surface it, so "This device" always appears. (2) **Team**: the persistent "Invitation sent" inline banner became a dismissable toast; the Pending-invitations list now renders from server state (fresh invites appear after `router.refresh()`) and each row gains **Resend** (new `resendInvitation` action, refreshes expiry); team members get a per-row **⋯ menu** (Make admin/member, Reset 2FA, Remove) via new `updateMemberRole` + `removeMember` actions (owner + self are protected). (3) **Profile**: dropped the redundant disabled Email input (the read-only Account card already shows it). (4) **Notifications**: verified the email defaults already match the agreed on-by-default set and `@mention` in-app is `inAppLocked` — made the locked-on cell render in a distinct muted+lock style so it no longer looks like an ordinary togglable checkbox. **Deferred:** Profile Language selector + avatar upload (need a `profiles.language` column / storage — no migration access). |
| A-2 (candidate columns) | More optional columns on the Candidates list | **Shipped 2026-07-03** — Per `FIX-PROMPT-EditCandidate-And-Columns` §3. "Manage columns" now offers **Stage** (badge, bucket-mapped to the Pipeline palette — derived from the candidate's active/first application), **Fit score** (that application's 0–100 evaluation score, shown as %), **Location**, **Salary expectation**, **Notice period**, and **Languages**, in addition to the existing set. Location/salary/notice/languages come straight off `candidates`; Stage joins `applications → pipeline_stages`, Fit score joins `candidate_evaluations`. `MAX_OPTIONAL_COLUMNS` (3-at-a-time) is unchanged. Edit stays a single-page form (not a wizard) per §2 — intentional. **Deferred:** org custom fields as addable columns — the column list is a static constant; making custom fields selectable needs dynamic column defs threaded through the toolbar + column-manager + saved prefs (a distinct change). |
| A-2 (name casing) | Candidate names show natural case on Edit | **Shipped 2026-07-03** — Edit candidate rendered ALL-CAPS names ("ALEKSANDRE") because CV-parsed/caps-locked values are stored uppercase and the form showed them raw. The form now runs initial + CV-parsed first/last names through `toDisplayName` (title-cases pure-uppercase ASCII, leaves mixed-case + non-Latin alone), so Edit shows natural case and saving persists it. The list + pipeline cards already normalised at display via `toDisplayFullName`/`toDisplayName`. |
...
- **Wave 3.1 AI Fit Analysis** — ~~blocked on Phase 0.8 legal consult~~ **Shipped 2026-07-22, opt-in + default OFF.** The code no longer blocks on the consult; instead the feature stays OFF until an owner explicitly enables it behind an EU-AI-Act acknowledgement gate. The Phase 0.8 legal review is now the _adopting org's_ pre-enable step, recorded by the acknowledgement.
