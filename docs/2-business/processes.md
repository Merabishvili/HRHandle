...
- 🔄 **(2026-09-25) Vacancies "Archived" tab now keys off `archived_at`, not the `archived` status.** The nightly `expire_past_vacancies()` cron closes past-`end_date` vacancies with **status=closed + `archived_at`**, so they matched neither the active tabs (which exclude `archived_at`) nor the status-based "Archived" tab → they vanished with no recovery path. The Archived tab now shows `archived_at IS NOT NULL` (both auto-expired and manually archived); the detail page opens archived vacancies so they can be reopened; and `updateVacancyStatus` keeps `archived_at` in lockstep with the status (archive sets it, any other status clears it) and drops a stale past `end_date` on reopen so the cron doesn't immediately re-archive.
- 🔄 **(2026-09-25) Candidate general-status is auto-derived (active/inactive/hired).** `active` (მიმდინარე) = ≥1 open application; **`inactive` (არააქტიური)** = no open application (imported / manually added without a vacancy / all apps closed); `hired` unchanged. Migration `20260925_candidate_inactive_status.sql` converts the vestigial `archived` status into `inactive` — which finally activates the long-dead Migration-052 sync trigger (it targeted a `code='inactive'` that was never seeded). `createCandidate` now stamps inactive (→ active only when a vacancy is linked); `createApplication` now **allows inactive candidates** (blocks only `hired`) and flips them to active on add; bulk import stamps imported candidates **inactive**. Apply the migration on both projects.
