...
| ~~**Interview questions**~~ | UI removed 2026-07-03 | `POST /api/ai/interview-questions` (route retained) | The separate "AI interview questions" section on the Scorecard & interview tab was retired (redesign S04) — it duplicated the Assessment suggester with a second Generate/Copy/Copy-all/Save mechanism. No UI entry point remains. The `/api/ai/interview-questions` route, `lib/ai/interview-questions.ts`, `lib/actions/interview-questions.ts`, and the `vacancies.interview_questions` JSONB column are retained pending the planned drop migration (`046_drop_vacancies_interview_questions.sql`). | Google Gemini | 100 / hour / org |
