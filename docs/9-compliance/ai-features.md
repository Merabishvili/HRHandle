...
| 2026-08-24 | **AI Fit async hardening.** The background `after()` run was still hitting Vercel's 60s function limit ("Task timed out after 60 seconds"), leaving the row stuck `pending` → the card spun forever. Added a hard **50s deadline** in `processFitAnalysis` (races the model; marks the row `failed`/`timeout` before the kill) + tightened the model budget (`FIT_TIMEOUT_MS` 25s→18s, `MAX_ATTEMPTS_PER_MODEL` 3→2). The card also stops the "generating…" spinner after a **5-min ceiling** and shows Retry. | Aleksandre Merabishvili |
