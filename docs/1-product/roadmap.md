...
| Area | Shipped | What |
|---|---|---|
| Main pipeline model | 2026-08-29 | The org-level stage set (renamed **Main pipeline** in Settings) now drives the cross-vacancy `/pipeline` board columns (custom names, not the fixed canonical 7; falls back to canonical when no Main pipeline). New `pipeline_stages.origin_template_id` links each per-vacancy stage to the Main-pipeline row it was cloned from → deterministic column roll-up; vacancy-only stages roll up by canonical bucket. Moves go through `moveApplicationToMainColumn` (origin-link resolution, robust to renamed stages). Main pipeline seeded at onboarding; a vacancy's Settings tab now labels its own stages "This vacancy's pipeline" + an "inherited from Main pipeline" badge. Candidate-profile stage tracker shows the app's real per-vacancy stage names. Migration `20260829_main_pipeline_origin_link` (apply on both projects). |
| AI features | G-009 → G-014 | Candidate summary (retired 2026-08-20), JD generator, interview questions, note-extractor (retired 2026-08-28), inclusive-language check, assessment suggester. G-015 email drafter shipped 2026-06-09 then retired 2026-06-21 (founder removed it — recruiter-managed templates already covered the real customer-facing email needs). Six design principles in [ai-features.md](../9-compliance/ai-features.md). |
...
| Assessment | — | **Assessment side sheet + permanent record — Phase 1 (2026-08-27).** Replaced the centred "Score candidate" modal with a right-anchored, single-scroll sheet (`components/candidates/profile/assessment-sheet.tsx`) that now **captures interview-question answers** (previously discarded — the `text_value` column existed but was never wired), **autosaves every field** (debounced + flushed on tab-hide/close, survives a browser close — `lib/scorecards/autosave.ts`), and has **no mandatory fields** (submit works on an empty card). Submitted assessments surface as a **permanent read-only record** on the candidate profile (left-column section + rail chip, `assessment-record.tsx`), persisting across all stages **including closed**, one card per assessor. Migration-free — rides existing `candidate_evaluations` columns. **Phase 2 (stepped 3-step wizard) intentionally deferred** pending real usage; the three sheet sections are discrete components so Phase 2 is an additive wrapper, not a rewrite. Dropped by decision: versioning, stage-at-submission badge, PDF/print, criterion↔question cross-link. |
| Support | — | **Support ticket flow (2026-08-29).** Subject + message + optional attachment form on two surfaces: in-app (`Settings → Support`, email/org derived from session) and public (`/support`, adds an email field + Turnstile). `submitSupportTicket` (`lib/actions/support.ts`) validates, stores an attachment in the private `support-attachments` bucket, inserts a `support_tickets` row (RLS-locked, service-role writes), and sends two Resend emails: a localized confirmation to the submitter (`From: support@hrhandle.com`) and a notification to `SUPPORT_INBOX` (`Reply-To:` the submitter). New migration `supabase/migrations/20260829_support_tickets.sql` (**apply on both projects**). `support@hrhandle.com` is an ImprovMX→Gmail alias with Resend outbound; contact address swapped from gmail across legal pages + docs in the same batch. |
