...
- 🔄 `GOOGLE_GEMINI_API_KEY` is now in `lib/env.ts` (optional) and used by `lib/cv-parser.ts` **and** the AI modules (`lib/ai/*` — note-extractor, bias-check, jd-generator, candidate-summary, assessment-suggester).
