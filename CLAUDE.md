...
- **MFA + password change/reset (AAL2)**: once a user has a verified MFA factor, Supabase requires an **AAL2** session to `updateUser({ password })` (errors `AAL2 session is required … when MFA is enabled` / code `insufficient_aal`). Two consequences: (1) **Change password** (`components/settings/change-password-form.tsx`) must NOT verify the current password via a client `signInWithPassword` — that mints a fresh **AAL1** session and downgrades the user's AAL2, breaking the update. Verify via the stateless server action `verifyPassword()` instead (no session touch), then `updateUser` runs on the intact AAL2 client. (2) **Reset password** (`app/auth/reset-password/page.tsx`) starts from an AAL1 recovery session, so for MFA accounts it prompts for a TOTP code and elevates to AAL2 (`startLoginChallenge`/`completeLoginChallenge`) before `updateUser`. The AAL2 error is localized via `authErrorMessage` → `auth.errMfaRequired`.
- **Change password requires `current_password`**: the Supabase projects enable `GOTRUE_SECURITY_UPDATE_PASSWORD_REQUIRE_CURRENT_PASSWORD`, so `updateUser({ password })` alone is rejected with `current_password_required` (400). The change-password form passes `updateUser({ password, current_password })` (supabase-js `UserAttributes.current_password`). The recovery/reset flow is exempt (the user has no current password), so `app/auth/reset-password/page.tsx` does NOT send it.
