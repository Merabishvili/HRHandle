...
const MAX_IQ_REQUESTS_PER_ORG_PER_HOUR = 100
...
function checkRateLimit(orgId: string): boolean {
  ...
  if (entry.count >= MAX_IQ_REQUESTS_PER_ORG_PER_HOUR) {
    return false
  }
  entry.count++
  return true
...
export async function POST(req: NextRequest) {
  ...
  // Fetch vacancy scoped to the user's org. Joins sectors to get a name.
  ...
  if (vErr) {
    console.error('[ai/interview-questions] vacancy fetch failed:', vErr.message)
